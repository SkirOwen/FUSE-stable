...
def error_check_structure(atoms, ideal_density, density_cutoff, check_bonds, btol,
	...
	if check_bonds == True:
		if system_type == 'neutral':
			...
			errors = test_bonds(atoms=atoms, cations=cations, anions=anions, charges=charges, ap=ap, lib=bondtable,
		...
		if system_type == 'ionic':
			...
			if check_anions == True:
				an_errors = test_bonds(atoms=atoms, cations=anions, anions=cations, charges=an_charges, ap=ap,
				                       lib=bondtable, system_type=system_type)
