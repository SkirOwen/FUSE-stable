	...
	while complete == False:
		...
		if move == 4:
			...
			for i in vacs:
				...
				while done == False:
					...
					if trials >= len(
							viable):  # seems reasonable to attmpt each of the listed vacencies before giving up!
						sites.append(i)
						done = True
		...
		if move == 11:  # Attmpt to grow the structure along an axis. here there's also the oppertunity to apply symmetry operations to the grown part of the cell, e.g. a mirror plane or inversion centre
			...
			if typ == "invert":
				...
				rep3 = [rep[0] - 1, rep[1] - 1,
				        rep[2] - 1]  # once we've inverted co-ordinates, how far we need to move the atom
			...
			accept = error_check_structure(temp_atoms, ideal_density, density_cutoff, check_bonds, btol, system_type,
			                               fu, composition, bondtable, ap, check_distances, dist_cutoff,
			                               target_number_atoms=target_number_atoms)
