...
def run_vasp(atoms='', vasp_opts='', kcut='', produce_steps='', dist_cutoff=1.0, ftol=0.5):
	short_contact = False
	converged = False
	energy = 1.e20
	...
	if short_contact == False:
		...
		for i in range(len(list(vasp_opts.keys()))):
			...
			kp = []
			kp = (int(math.ceil(temp_kcut / cell[0])), int(math.ceil(temp_kcut / cell[1])),
		...
		if calc.int_params['nsw'] > 0:
			if converged == False:
				fo = new_atoms.get_forces()
				max_fo = abs(fo.max())
				print("max forces: ", max_fo)
				if max_fo > ftol:
					converged = False
					energy = 1.e20

	if short_contact == True:
		converged = False
		energy = 1.e20
