...
def assemble_structure(string, instructions):
	...
	for i in range(nsub):
		...
		for j in range(4):
			if sub_mods[i][j] != 120:
				tmp_atoms = Atoms(numbers=[sub_mods[i][j]], pbc=[1, 1, 1],
				                  cell=[ap, ap, ap * 0.5, angles[0], angles[1], angles[2]],
				                  scaled_positions=[co_ords[j]])
				if atoms != None:
	...
	for i in range(full[2]):
		if i == 0:
			atoms = mods[i]
			atoms.translate(([ptrans[translations[i]][0] * ap, ptrans[translations[i]][1] * ap,
			                  ptrans[translations[i]][2] * (ap / 2)]))
		else:
			temp_atoms = mods[i]
			temp_atoms.translate(atoms.cell[2])
			temp_atoms.translate(([ptrans[translations[i]][0] * ap, ptrans[translations[i]][1] * ap,
			                       ptrans[translations[i]][2] * (ap / 2)]))
			atoms += temp_atoms
